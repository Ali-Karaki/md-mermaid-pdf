...
/** Preset CSS lives next to package.json (avoids broken paths if `__dirname` differs after bundling). */
function presetStylesheetPath(filename) {
	const root = path.dirname(require.resolve('md-mermaid-pdf/package.json'));
	return path.join(root, 'presets', filename);
}
...
async function mdToPdf(input, userConfig = {}) {
	...
	const presetStylesheet =
		preset === 'github' || preset === 'minimal' || preset === 'slides'
			? [presetStylesheetPath(`${preset}.css`)]
			: [];
	const documentThemeStylesheet =
		documentTheme === 'dark' ? [presetStylesheetPath('document-dark.css')] : [];
