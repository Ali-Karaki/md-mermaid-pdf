...
async function generateOutput(html, relativePath, config, browserRef) {
	...
	if (mermaidResult?.error) {
		const count = mermaidResult.count ?? 1;
		const msg =
			count > 1
				? `Mermaid parse error (one of ${count} diagrams): ${mermaidResult.error}`
				: `Mermaid parse error: ${mermaidResult.error}`;
		const err = new Error(msg);
		err.name = 'MermaidParseError';
		err.diagramCount = count;
		err.originalMessage = mermaidResult.error;
		if (typeof onMermaidError === 'function') {
			const action = await onMermaidError(err, { diagramCount: count });
			if (action === 'skip') {
				/* continue */
			} else {
				throw err;
